...
const Login = () => {
  ...
  const { runAsync: login, loading } = useRequest(loginService.login, {
    manual: true,
  });
  const { data: captcha, refresh: refreshCaptcha } = useRequest(
    loginService.getCaptcha
  );
  const { runAsync: getPublicKey } = useRequest(loginService.getPublicKey, {
    manual: true,
  });
  const { runAsync: sendResetPasswordEmail, loading: resetPasswordBtnLoading } =
    useRequest(loginService.sendResetPasswordEmail, { manual: true });
    ...
    <div className="bg-primary light:bg-[rgb(238,242,246)] bg-[rgb(238,242,246)] flex justify-center items-center h-[100vh]">
        ...
        <div className="dark:bg-[rgb(33,41,70)] w-[400px] px-[32px] py-[20px] mt-[-12%] bg-white rounded-lg <lg:(w-[94%] mx-auto)">
          ...
          >
            <Form.Item
              name="accountNumber"
              rules={[
                { required: true, message: t("wVzXBuYs" /* 请输入账号 */) },
              ]}
            >
              <Input
            ...
            <Form.Item
              name="password"
              rules={[
                { required: true, message: t("DjMcEMAe" /* 请输入密码 */) },
              ]}
            >
              <Input
      ...
      >
        ...
        <Input
          ...
          placeholder="请输入邮箱"
          onBlur={() => {
            setEmailInputFoucs(false);
          }}
          onFocus={() => {
            setEmailInputFoucs(true);
          }}
          onChange={(e) => {
